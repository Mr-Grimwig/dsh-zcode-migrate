...
/**
 * Event-data members excluded from a content digest.
 *
 * `stream` is the timing of an assistant message's deltas, not the message: the
 * runtime reslices it when it migrates a stored log to a newer format (a
 * migrated message legitimately arrives as one run per block, a natively
 * recorded one as hundreds). Digesting it would make every session look
 * rewritten the first time the harness touches it.
 */
const NON_CONTENT_MEMBERS = new Set(['stream']);

/**
 * Project one event down to its content identity.
 * @param {object} event - session event.
 * @returns {object} `{ type, data }` with non-content members removed.
 */
function contentProjection(event) {
  const data = event.data;
  if (data === null || typeof data !== 'object') return { type: event.type, data };
  /** @type {object} */
  const kept = {};
  for (const key of Object.keys(data)) {
    if (NON_CONTENT_MEMBERS.has(key)) continue;
    kept[key] = data[key];
  }
  return { type: event.type, data: kept };
}
  ...
  const close = () => {
    ...
    turns.push({
      ...
      digest:
        content.length === 0
          ? ''
          : sha256(stableStringify(content.map((event) => contentProjection(event)))),
