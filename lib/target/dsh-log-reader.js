...
/**
 * Physical artifact names a session log can carry.
 *
 * Generation 0 uses the bare `session.jsonl[.zstd]`; every later generation
 * embeds its format version (`session.v3.jsonl.zstd`), so both spellings have to
 * be recognized by anything that reads storage directly.
 */
const ARTIFACT_PATTERN = /^session(?:\.v(\d+))?\.jsonl(\.zstd)?$/;

/**
 * Whether a file name is a session artifact, and which format it declares.
 * @param {string} name - base file name.
 * @returns {{ version: number|undefined }|undefined} format hint, or `undefined` when it is not an artifact.
 */
export function parseArtifactName(name) {
  const match = ARTIFACT_PATTERN.exec(name);
  if (match === null) return undefined;
  return { version: match[1] === undefined ? undefined : Number(match[1]) };
}
...
/**
 * List every session artifact under a sessions root, newest generation first.
 *
 * A session directory can hold more than one generation: migrating an older log
 * writes the new format beside it (`session.v3.jsonl.zstd` next to
 * `session.jsonl.zstd`) and leaves the previous one in place. Only the newest is
 * authoritative, hence the sort.
 *
 * @param {string} root - the DSH sessions root (`<dsh home>/sessions`).
 * @returns {Array<{ sessionId: string, path: string, size: number, projectDir: string, formatVersion?: number }>} artifacts found.
 */
export function listArtifacts(root) {
  if (!existsSync(root)) return [];
  /** @type {Array<{ sessionId: string, path: string, size: number, projectDir: string, formatVersion?: number }>} */
  const found = [];
  let projectDirs;
  try {
  ...
  for (const projectDir of projectDirs) {
    ...
    for (const sessionDir of sessionDirs) {
      // Read the directory instead of probing fixed names: a newer generation
      // embeds its format version in the file name, and the next one will too.
      let files;
      try {
        files = readdirSync(join(projectPath, sessionDir.name));
      } catch {
        continue;
      }
      for (const file of files) {
        ...
        const path = join(projectPath, sessionDir.name, file);
        try {
          found.push({
            ...
            // 不带版本号的文件名按约定就是第 0 代，所以这个字段总是数字
            formatVersion: parsed.version ?? 0,
          });
        } catch {
  ...
  // 新一代优先：迁移新写的日志与旧代并存时，只有最新那份是权威
  return found.sort((left, right) => {
    const byVersion = right.formatVersion - left.formatVersion;
    return byVersion !== 0 ? byVersion : left.path.localeCompare(right.path);
  });
