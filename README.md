...
在 Windows 上对着 247 个真实会话做过完整验证：导入 247/247 成功，21151 段思考（约 5534 万字）逐字比对全部一致，没有任何冲突或失败。测试 162 项。

DSH 换过一次会话日志格式（0.1.0 的 v0 到 0.1.5 的 v3：`assistant/message` 改为内嵌 stream 记录，头部用 `isSeeded` 取代 `seedLength`，产物文件也从 `session.jsonl.zstd` 变成 `session.v3.jsonl.zstd`），插件按后端实际提供的接口自动判断该写哪种，两代都在测试里跑过：同一套 162 项测试分别对 v0 后端和 v3 后端各过一遍。旧日志不需要转换，新版 harness 读得进去，会在下次追加时自己迁移。
...
最省事的是双击安装包：到 [Releases](https://github.com/Mr-Grimwig/dsh-zcode-migrate/releases/latest) 下载 `install.cmd`（不到 1KB），双击运行。它会下载当前 main 分支到 `%LOCALAPPDATA%\dsh-zcode-migrate`，再装进你机器上所有 dsh profile。跑完重启一次 DSH 就行。再双击一次就是更新到最新版，卸载用同一个 Release 里的 `uninstall.cmd`。

需要 Windows、DSH，以及 Node.js 22.15 以上（没装的话脚本会提示去哪装）。
