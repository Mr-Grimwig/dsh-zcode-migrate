...
在 Windows 上对着 247 个真实会话做过完整验证：导入 247/247 成功，21151 段思考（约 5534 万字）逐字比对全部一致，没有任何冲突或失败。测试 161 项。

DSH 换过一次会话日志格式（0.1.0 的 v0 到 0.1.5 的 v3：`assistant/message` 改为内嵌 stream 记录，头部用 `isSeeded` 取代 `seedLength`，产物文件也从 `session.jsonl.zstd` 变成 `session.v3.jsonl.zstd`），插件按后端实际提供的接口自动判断该写哪种，两代都在测试里跑过：同一套 161 项测试分别对 v0 后端和 v3 后端各过一遍。旧日志不需要转换，新版 harness 读得进去，会在下次追加时自己迁移。
